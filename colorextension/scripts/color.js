	...
	if (data.message == "loaded") {
		...
		imagesList.forEach((image, index) => {
				...
				for (let index = 0; index < imageInfoLength; index += 4) {
					...
					if (index != 0) {
						...
						if (thisData[index - 4] - thisData[index] <= 48) {
							...
							thisData[index] - thisData[index - 4] <=
							48
						) {
							thisData[index] =
								factor * (thisData[index] - 128) + 128;
						}
						// ex. #FAA0A0 TO FAD000 (A-D: contrast able to see) 0D0h - 0A0h = 48d = 30h
						if (
							thisData[index - 4 + 1] - thisData[index + 1] <=
							48
						) {
							thisData[index - 4 + 1] =
								factor * (thisData[index - 4 + 1] - 128) + 128;
						} else if (
							thisData[index] - thisData[index - 4 + 1] <=
							48
						) {
							thisData[index + 1] =
								factor * (thisData[index + 1] - 128) + 128;
			...
			const deutan = () => {
				...
				for (let index = 0; index < imageInfoLength; index += 4) {
					...
					if (index != 0) {
						...
						// ex. #FAA0A0 TO FAD000 (A-D: contrast able to see) 0D0h - 0A0h = 48d = 30h
						if (
							thisData[index - 4 + 2] - thisData[index + 2] <=
							48
						) {
							thisData[index - 4 + 2] =
								factor * (thisData[index - 4 + 2] - 128) + 128;
						} else if (
							thisData[index] - thisData[index - 4 + 2] <=
							48
						) {
							thisData[index + 2] =
								factor * (thisData[index + 2] - 128) + 128;
				...
				for (let index = 0; index < imageInfoLength; index += 4) {
					...
					if (index != 0) {
						...
						if (
							...
							thisData[index + 1] - thisData[index - 4 + 1] <=
							48
						) {
							thisData[index + 1] =
								factor * (thisData[index + 1] - 128) + 128;
			...
			const sliderChangeGreen = (valueG) => {
				...
				document.getElementById("GreenValue").innerText =
					greenSlider.value;

				const imageData = canvasContent.getImageData(
					0,
					0,
					thisCanvas.width,
					thisCanvas.height
			...
			const sliderChangeBlue = (valueB) => {
				...
				document.getElementById("BlueValue").innerText =
					blueSlider.value;
			...
			savePresetBtn.addEventListener("click", (event) => {
				...
				let allSettings = {
					...
					redSlider: parseInt(redSlider.value),
					blueSlider: parseInt(blueSlider.value),
					greenSlider: parseInt(greenSlider.value),
			...
			loadPresetBtn.addEventListener("click", (event) => {
				...
				if (retrieved.redSlider != 0) {
					redSlider.value = retrieved.redSlider;
					sliderChangeRed(redSlider.value);
				}
				if (retrieved.greenSlider != 0) {
					greenSlider.value = retrieved.greenSlider;
					sliderChangeGreen(greenSlider.value);
				}
				if (retrieved.blueSlider != 0) {
					blueSlider.value = retrieved.blueSlider;
					sliderChangeBlue(blueSlider.value);
				}
